// AllDonationRequestVolunteerRole

import { Link } from "react-router-dom";
import useAllDonaRequest from "../../hooks/useAllDonaRequest";
import { useEffect, useState } from "react";
import useAxiosSecure from "../../hooks/useAxiosSecure";
import Swal from "sweetalert2";

const AllDonationRequestVolunteerRole = () => {
    const [allRequests, refetch] = useAllDonaRequest();
    const [requestList, setRequestList] = useState(allRequests);
    ...
    const sortStatus = (status) => {
        if (status !== 'all') {
            const filteredRequests = allRequests.filter(request => request.donation_status === status);
    ...
    return (
        <div className="bg-[#dad7cd] py-12">
            <h1 className="text-4xl text-[#000814] font-bold text-center">All Donation Requests</h1>
...
export default AllDonationRequestVolunteerRole;